...
import { createProxiedUrl, definitelyUrl, isBskyAppUrl, isBskyStarterPackUrl, isExternalUrl, isPossiblyAUrl, isShortLink, isValidDomain, labelToDomain, shortLinkToHref, toShortUrl } from "#/lib/strings/url-helpers"
import {
  isTrustedUrl,
  linkRequiresWarning,
  splitApexDomain,
...
// src/lib/__tests__/bskyUtils.test.ts



describe('bskyUtils', () => {
  test('isValidDomain returns true for known valid domains', () => {
  ...
  test('isBskyAppUrl correctly identifies bsky.app URLs', () => {
    expect(isBskyAppUrl('https://bsky.app/profile/user')).toBe(true)
    expect(isBskyAppUrl('https://otherdomain.com')).toBe(false)
    expect(isBskyAppUrl('not a url')).toBe(false)
  })

  test('toShortUrl truncates long URLs correctly', () => {
    const longUrl = 'https://bsky.app/path/to/resource/with/query?test=true'
    expect(toShortUrl(longUrl)).toContain('...')
  })

  test('isExternalUrl detects external URLs properly', () => {
    expect(isExternalUrl('https://example.com')).toBe(true)
    expect(isExternalUrl('/rss')).toBe(true)
    expect(isExternalUrl('https://bsky.app/profile')).toBe(false)
    expect(isExternalUrl('bsky.app/profile')).toBe(false)
  })

  test('createProxiedUrl builds a valid proxied URL', () => {
    const input = 'https://example.com/test'
    const output = createProxiedUrl(input)
    expect(output).toBe('https://go.bsky.app/redirect?u=https%3A%2F%2Fexample.com%2Ftest')
  })

  test('isShortLink recognizes short bsky links', () => {
    expect(isShortLink('https://go.bsky.app/abc123')).toBe(true)
    expect(isShortLink('https://bsky.app/profile')).toBe(false)
    expect(isShortLink('')).toBe(false)
  })

  test('shortLinkToHref transforms short links to expected paths', () => {
    expect(shortLinkToHref('https://go.bsky.app/starterpack123')).toBe('/starter-pack-short/starterpack123')
  })

  test('isBskyStarterPackUrl detects starter-pack links', () => {
    expect(isBskyStarterPackUrl('https://bsky.app/starter-pack/johndoe/abc123')).toBe(true)
    expect(isBskyStarterPackUrl('https://bsky.app/other')).toBe(false)
    expect(isBskyStarterPackUrl('https://bsky.app/starter-pack')).toBe(false)
  })

  test('labelToDomain extracts domains from labels', () => {
    expect(labelToDomain('https://example.com')).toBe('example.com')
    expect(labelToDomain('example.com')).toBe('example.com')
    expect(labelToDomain('not a url')).toBeUndefined()
  })

  test('isPossiblyAUrl detects URL-like strings', () => {
    expect(isPossiblyAUrl('example.com')).toBe(true)
    expect(isPossiblyAUrl('ftp://file')).toBe(false)
    expect(isPossiblyAUrl('not@url')).toBe(false)
  })

  test('definitelyUrl validates and formats URLs correctly', () => {
    expect(definitelyUrl('example.com')).toBe('https://example.com/')
    expect(definitelyUrl('not_a_url')).toBeNull()
    expect(definitelyUrl('example.com')).toBe('https://example.com/')
  })
})
