...
const withBitdriftAppBuildGradle = (config, props) => {
...
const withBitdriftSettingsGradle = (config, _props) => {
  ...
  return (0, _configPlugins.withSettingsGradle)(config, config => {
    if (!config.modResults.contents.includes('dl.bitdrift.io')) {
      ...
      config.modResults.contents = config.modResults.contents.replace('pluginManagement {', `pluginManagement {
    repositories {
        mavenCentral()
        gradlePluginPortal()
        maven {
...
const withAndroid = (config, props) => {
  return (0, _configPlugins.withPlugins)(config, [[withBitdriftAppBuildGradle, props], [withBitdriftSettingsGradle, props]]);
};
var _default = exports.default = withAndroid;
