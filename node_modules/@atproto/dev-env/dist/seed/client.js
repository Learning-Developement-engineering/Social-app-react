...
class SeedClient {
    ...
    async createProfile(by, displayName, description, selfLabels, joinedViaStarterPack) {
    ...
    async reply(by, root, parent, text, facets, images) {
        var _a;
        const embed = images
            ? {
                $type: 'app.bsky.embed.images',
    ...
    async repost(by, subject) {
        var _a;
        const res = await this.agent.app.bsky.feed.repost.create({ repo: by }, { subject: subject.raw, createdAt: new Date().toISOString() }, this.getHeaders(by));
