...
class Server {
    constructor(lexicons, opts = {}) {
        ...
        Object.defineProperty(this, "routeRateLimiters", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
        ...
        this.options = opts;
        this.middleware = {
            json: (0, express_1.json)({ limit: opts?.payload?.jsonLimit }),
            text: (0, express_1.text)({ limit: opts?.payload?.textLimit }),
        };
    ...
    createHandler(nsid, def, routeCfg) {
        ...
        const consumeRateLimit = (reqCtx) => (0, rate_limiter_1.consumeMany)(reqCtx, rls.map((rl) => (ctx) => rl.consume(ctx)));
        const resetRateLimit = (reqCtx) => (0, rate_limiter_1.resetMany)(reqCtx, rls.map((rl) => (ctx) => rl.reset(ctx)));
        return async function (req, res, next) {
            try {
                ...
                else if ((0, types_1.isHandlerError)(output)) {
                    next(types_1.XRPCError.fromError(output));
                }
                else {
                    validateResOutput?.(output);
                    res.status(200);
            ...
            handler: async function* (req, signal) {
                try {
                    ...
                    if ((0, types_1.isHandlerError)(auth)) {
                        throw types_1.XRPCError.fromHandlerError(auth);
                    }
                    // validate request
        ...
        if (config.rateLimit) {
            ...
            for (let i = 0; i < limits.length; i++) {
                ...
                if ((0, types_1.isShared)(limit)) {
                    ...
                    if (rateLimiter) {
                        this.routeRateLimiters[nsid].push({
                            consume: (ctx) => rateLimiter.consume(ctx, {
                                calcKey,
                                calcPoints,
                            }),
                            reset: (ctx) => rateLimiter.reset(ctx, {
                                calcKey,
                            }),
                        });
                    }
    ...
    return async function (req, res, next) {
        try {
            ...
            if ((0, types_1.isHandlerError)(result)) {
                throw types_1.XRPCError.fromHandlerError(result);
            }
            const locals = req[kRequestLocals];
            locals.auth = result;
            next();
