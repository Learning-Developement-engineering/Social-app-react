...
function default_1(server, ctx) {
    const listNotifications = (0, pipeline_1.createPipeline)(skeleton, hydration, noBlockOrMutesOrNeedsReview, presentation);
    server.app.bsky.notification.listNotifications({
        ...
        handler: async ({ params, auth, req }) => {
            ...
            const result = await listNotifications({ ...params, hydrateCtx: hydrateCtx.copy({ viewer }) }, ctx);
            return {
...
const noBlockOrMutesOrNeedsReview = (input) => {
    ...
    skeleton.notifs = skeleton.notifs.filter((item) => {
        ...
        if (item.reason === 'reply') {
            ...
            if (post) {
                const rootPostUri = (0, post_1.isRecord)(post.record)
                ...
                const isHiddenByThreadgate = isRootPostByViewer
                    ? ctx.views.replyIsHiddenByThreadgate(item.uri, rootPostUri, hydration)
                    : false;
