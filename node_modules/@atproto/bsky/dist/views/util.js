...
const parseThreadGate = (replierDid, ownerDid, rootPost, gate) => {
    ...
    const allowFollower = gate.allow.some(threadgate_1.isFollowerRule);
    ...
    if (allowMentions) {
        const isMentioned = rootPost?.facets?.some((facet) => {
            return facet.features.some((item) => (0, facet_1.isMention)(item) && item.did === replierDid);
        });
        if (isMentioned) {
            return {
                canReply: true,
