...
class HydrateCtx {
    constructor(vals) {
        ...
        Object.defineProperty(this, "includeActorTakedowns", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: this.vals.includeActorTakedowns
        });
...
class Hydrator {
    ...
    //     - profile
    //       - list basic
    async hydratePosts(refs, ctx, state = {}) {
        ...
        const postsLayer0 = await this.feed.getPosts(uris, ctx.includeTakedowns, state.posts);
        ...
        const embedPostUrisLayer1 = urisLayer1ByCollection.get(lexicons_1.ids.AppBskyFeedPost) ?? [];
        ...
        const embedPostUrisLayer2 = urisLayer2ByCollection.get(lexicons_1.ids.AppBskyFeedPost) ?? [];
        ...
        const nestedListUris = [
            ...(urisLayer1ByCollection.get(lexicons_1.ids.AppBskyGraphList) ?? []),
            ...(urisLayer2ByCollection.get(lexicons_1.ids.AppBskyGraphList) ?? []),
        ];
        const nestedFeedGenUris = [
            ...(urisLayer1ByCollection.get(lexicons_1.ids.AppBskyFeedGenerator) ?? []),
            ...(urisLayer2ByCollection.get(lexicons_1.ids.AppBskyFeedGenerator) ?? []),
        ];
        const nestedLabelerDids = [
            ...(urisLayer1ByCollection.get(lexicons_1.ids.AppBskyLabelerService) ?? []),
            ...(urisLayer2ByCollection.get(lexicons_1.ids.AppBskyLabelerService) ?? []),
        ].map(uris_1.uriToDid);
        const nestedStarterPackUris = [
            ...(urisLayer1ByCollection.get(lexicons_1.ids.AppBskyGraphStarterpack) ?? []),
            ...(urisLayer2ByCollection.get(lexicons_1.ids.AppBskyGraphStarterpack) ?? []),
        ];
    ...
    async hydrateStarterPacks(uris, ctx) {
        ...
        uris.forEach((uri) => {
            ...
            agg.listItemSampleUris = [
                ...members.listitems.filter((li) => ctx.viewer === creator || !isBlocked(blocks, [creator, li.did])),
            ]
                .sort((li1, li2) => {
    ...
    async hydrateNotifications(notifs, ctx) {
        ...
        const postUris = collections.get(lexicons_1.ids.AppBskyFeedPost) ?? [];
        const likeUris = collections.get(lexicons_1.ids.AppBskyFeedLike) ?? [];
        const repostUris = collections.get(lexicons_1.ids.AppBskyFeedRepost) ?? [];
        const followUris = collections.get(lexicons_1.ids.AppBskyGraphFollow) ?? [];
        const verificationUris = collections.get(lexicons_1.ids.AppBskyGraphVerification) ?? [];
    ...
    async getRecord(uri, includeTakedowns = false) {
        ...
        else if (collection === lexicons_1.ids.AppBskyGraphListitem) {
            return ((await this.graph.getListItems([uri], includeTakedowns)).get(uri) ??
                undefined);
        }
        else if (collection === lexicons_1.ids.AppBskyGraphBlock) {
            return ((await this.graph.getBlocks([uri], includeTakedowns)).get(uri) ??
                undefined);
        }
        else if (collection === lexicons_1.ids.AppBskyGraphStarterpack) {
        ...
        else if (collection === lexicons_1.ids.AppBskyFeedThreadgate) {
            return ((await this.feed.getThreadgateRecords([uri], includeTakedowns)).get(uri) ?? undefined);
        }
        else if (collection === lexicons_1.ids.AppBskyFeedPostgate) {
            return ((await this.feed.getPostgateRecords([uri], includeTakedowns)).get(uri) ?? undefined);
        }
        else if (collection === lexicons_1.ids.AppBskyLabelerService) {
            if (parsed.rkey !== 'self')
                return;
        ...
        else if (collection === lexicons_1.ids.ChatBskyActorDeclaration) {
            if (parsed.rkey !== 'self')
                return;
        ...
        else if (collection === lexicons_1.ids.AppBskyActorProfile) {
            const did = parsed.hostname;
...
const labelSubjectsForDid = (dids) => {
    return [
        ...dids,
        ...dids.map((did) => syntax_1.AtUri.make(did, lexicons_1.ids.AppBskyActorProfile, 'self').toString()),
    ];
};
...
const nestedRecordUris = (post) => {
    ...
    if (!post?.embed)
        return uris;
    if ((0, record_1.isMain)(post.embed)) {
        uris.push(post.embed.record.uri);
    }
    else if ((0, recordWithMedia_1.isMain)(post.embed)) {
...
const getListUrisFromThreadgates = (gates) => {
    ...
    for (const gate of gates.values()) {
        const listRules = gate?.record.allow?.filter(threadgate_1.isListRule) ?? [];
