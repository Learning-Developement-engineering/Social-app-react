...
const withIconImages = (config, { icons, dimensions }) => {
    return (0, config_plugins_1.withDangerousMod)(config, [
        ...
        async (config) => {
            ...
            await fs_1.default.promises
                .rm(path_1.default.join(iosRoot, IOS_FOLDER_NAME), {
                recursive: true,
                force: true,
            })
                .catch(() => null);
            // Ensure directory exists
            await fs_1.default.promises.mkdir(path_1.default.join(iosRoot, IOS_FOLDER_NAME), {
                recursive: true,
            });
            // Generate new assets
            await iterateIconsAndDimensionsAsync({ icons, dimensions }, async (key, { icon, dimension }) => {
                if (!icon.ios)
                    return;
                const iconFileName = getIconFileName(key, dimension);
                const fileName = path_1.default.join(IOS_FOLDER_NAME, iconFileName);
