...
var Evaluator = /** @class */ (function () {
    function Evaluator(fetcher, options) {
        this.initialized = false;
        this.store = new SpecStore_1["default"](fetcher, options);
    ...
    Evaluator.prototype.getConfig = function (user, configName, ctx) {
    ...
    Evaluator.prototype.getClientInitializeResponse = function (inputUser, _ctx, clientSDKKey, options) {
        var _this = this;
        var _a, _b, _c, _d;
        ...
        var ctx = targetAppID ? _ctx.withTargetAppID(targetAppID) : _ctx;
        ...
        var gates = Object.entries(this.store.getAllGates())
            ...
            var localOverride = (options === null || options === void 0 ? void 0 : options.includeLocalOverrides)
                ...
                : null;
            var res = localOverride !== null && localOverride !== void 0 ? localOverride : _this._eval(user, spec, ctx);
            return {
                name: (0, Hashing_1.hashString)(gate, options === null || options === void 0 ? void 0 : options.hash),
                value: res.unsupported ? false : res.value,
                rule_id: res.rule_id,
                secondary_exposures: _this.hashSecondaryExposure(res.secondary_exposures, options === null || options === void 0 ? void 0 : options.hash)
            };
        });
    ...
    Evaluator.prototype._evalSpec = function (user, config, ctx) {
        var evaulation = this._eval(user, config, ctx);
    ...
    Evaluator.prototype._eval = function (user, config, ctx) {
        if (!config.enabled) {
            return new ConfigEvaluation_1["default"](false, 'disabled', null, config.idType, [], config.defaultValue);
        }
        ...
        for (var i = 0; i < config.rules.length; i++) {
            ...
            if (ruleResult.value === true) {
                var delegatedResult = this._evalDelegate(user, rule, secondary_exposures, ctx);
                if (delegatedResult) {
        ...
        return new ConfigEvaluation_1["default"](false, 'default', null, config.idType, secondary_exposures, config.defaultValue, config.explicitParameters);
    };
    Evaluator.prototype._evalDelegate = function (user, rule, exposures, ctx) {
        ...
        var delegatedResult = this._eval(user, config, ctx);
    ...
    Evaluator.prototype._evalCondition = function (user, condition, ctx) {
        ...
        switch (condition.type.toLowerCase()) {
            ...
            case 'ip_based':
                ...
                value = (_d = getFromUser(user, field)) !== null && _d !== void 0 ? _d : this.getFromIP(user, field);
                break;
            case 'ua_based':
                // this would apply to things like 'os', 'browser', etc.
                value = (_e = getFromUser(user, field)) !== null && _e !== void 0 ? _e : getFromUserAgent(user, field);
                break;
            case 'user_field':
            ...
            case 'user_bucket': {
                ...
                var userHash = computeUserHash((_g = salt + '.' + this._getUnitID(user, idType)) !== null && _g !== void 0 ? _g : '');
    ...
    Evaluator.prototype._isUserAllocatedToExperiment = function (user, experimentConfig, ctx) {
        ...
        var evalResult = this._eval(user, experimentConfig, ctx);
